...
// Load 8-bit integer into an existing vector at the position specified by Index
template <u8 Index>
...
// Load 32-bit integer into an existing vector at the position specified by Index
template <u8 Index>
...
{
	static_assert(std::ranges::none_of(Control.m_data, [](s8 i){ return i >= 16; }), "All indices must be in the range [0, 15] or negative, since PSHUFB and TBL behave differently otherwise");
#if defined(__SSSE3__)
	return _mm_shuffle_epi8(vec, static_cast<v128>(Control));
#elif defined(ARCH_ARM64)
	return vqtbl1q_s8(vec, static_cast<v128>(Control));
#else
	v128 ret;
	for (s32 i = 0; i < 16; i++)
		ret._s8[i] = Control[i] < 0 ? 0 : vec._s8[Control[i] & 0xf];
	return ret;
#endif
}
