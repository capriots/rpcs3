...
static error_code notify_es_au_found(ppu_thread& ppu, vm::ptr<void> core_es_handle, vm::cptr<DmuxAuInfo> au_info, vm::ptr<DmuxEsContext> es_handle)
{
	...
	const auto notify_fatal_err = [&](be_t<u32> es_is_enabled, error_code ret)
	{
		if (es_is_enabled)
		{
	...
	// This is frequently checked in here since a different thread can disable the elementary stream via cellDmuxDisableEs().
	...
	{
		notify_fatal_err(es_handle->is_enabled, ret);
		return 1;
	}

	// Check if the access unit queue is full. One slot is reserved for flushing the stream so that it always succeeds.
	...
	DmuxAuInfo& _au_info = au_queue_data[es_handle->au_queue.back].au_info;
...
static vm::cptr<CellDmuxCoreOps> get_core_ops()
{
...
error_code cellDmuxQueryEsAttr(ppu_thread& ppu, vm::cptr<CellDmuxType> demuxerType, vm::cptr<CellCodecEsFilterId> esFilterId, vm::cptr<void> esSpecificInfo, vm::ptr<CellDmuxEsAttr> esAttr)
{
	...
	esAttr->memSize = utils::align<u32, u32>(sizeof(DmuxEsContext) + ((core_es_attr->auQueueMaxSize + 1) * (core_es_attr->specificInfoSize + sizeof(DmuxAuQueueData))), alignof(DmuxEsContext))
	...
	vm::cptr<CellDmuxEsCb> esCb, vm::cptr<void> esSpecificInfo, vm::pptr<DmuxEsContext> esHandle)
{
	...
	const auto core_mem_addr = vm::bptr<void>::make(utils::align<u32>(es_handle.addr() + sizeof(DmuxEsContext) + au_queue_data_size, 0x10));
