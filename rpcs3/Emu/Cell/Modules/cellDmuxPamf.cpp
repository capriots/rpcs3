...
void dmux_pamf_context::elementary_stream::output_queue::pop_back(u32 size)
{
	ensure(size <= back - buffer.data(), "Invalid au_size");
	back -= size;
}

void dmux_pamf_context::elementary_stream::output_queue::pop_back(std::byte* addr)
{
	ensure(addr >= buffer.data() && addr < std::to_address(buffer.end()), "Invalid au_begin");
	wrap_pos = addr <= back ? wrap_pos : buffer.data(); // If au_begin is in front of the back pointer, unwrap the back pointer (there are no more access units behind the back pointer)
	back = addr;
}

void dmux_pamf_context::elementary_stream::output_queue::pop_front(u32 size)
{
	front += size;
...
void dmux_pamf_context::elementary_stream::output_queue::push(const access_unit_chunk& au_chunk)
{
...
void dmux_pamf_context::elementary_stream::flush_es()
{
	if (current_au.accumulated_size != 0)
	{
		...
		ctx.event_queue_too_full = !ctx.send_event(DmuxPamfEventType::au_found, stream_id, private_stream_id, vm::get_addr(au_queue.peek_back(current_au.accumulated_size)), std::bit_cast<CellCodecTimeStamp>(static_cast<be_t<u64>>(current_au.pts)),
...
bool dmux_pamf_context::elementary_stream::process_stream_chunk()
{
	for (;;)
	{
		switch (state)
		...
		case state::initial:
			...
			stream = stream.subspan(parse_stream(stream));
		...
		case state::notifying_au_found:
			if (current_au.state == access_unit::state::complete && !ctx.send_event(DmuxPamfEventType::au_found, stream_id, private_stream_id, vm::get_addr(au_queue.peek_back(current_au.accumulated_size)),
				std::bit_cast<CellCodecTimeStamp>(static_cast<be_t<u64>>(current_au.pts)), std::bit_cast<CellCodecTimeStamp>(static_cast<be_t<u64>>(current_au.dts)), 0, current_au.accumulated_size,
				au_specific_info_size, current_au.au_specific_info_buf, es_id, current_au.rap))
			{
	...
	[&]
	{
		...
		for (; stream_it <= stream.end() - sizeof(u32); stream_it++) // LLE uses sizeof(u32), even though we only read sizeof(u16) bytes
		{
			...
			else if (tmp == SYNC_WORD)
			{
				...
				else if (const u32 au_size = current_au.accumulated_size + stream_it - au_chunk_begin + cache.size(); au_size >= current_au.parsed_size)
				{
					current_au.state = au_size == current_au.parsed_size ? access_unit::state::complete : access_unit::state::size_mismatch;
					return;
				}
...
u32 dmux_pamf_context::user_data_stream::parse_stream(const std::span<const std::byte>& stream)
{
	...
	return static_cast<u32>(stream.size());
}

void dmux_pamf_context::demuxer::enable_es(u32 stream_id, u32 private_stream_id, bool is_avc, const std::span<std::byte>& au_queue_buffer, u32 au_max_size, bool raw_es, u32 es_id)
{
	...
	case DMUX_PAMF_STREAM_TYPE_INDEX_VIDEO:
		elementary_streams[0][channel] = is_avc ? static_cast<std::unique_ptr<elementary_stream>>(std::make_unique<video_stream<true>>(ctx, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size))
			: std::make_unique<video_stream<false>>(ctx, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size);
		return;

	case DMUX_PAMF_STREAM_TYPE_INDEX_LPCM:      elementary_streams[1][channel] = std::make_unique<lpcm_stream>(ctx, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_AC3:       elementary_streams[2][channel] = std::make_unique<audio_stream<true>>(ctx, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_ATRACX:    elementary_streams[3][channel] = std::make_unique<audio_stream<false>>(ctx, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_USER_DATA: elementary_streams[4][channel] = std::make_unique<user_data_stream>(ctx, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size); return;
...
void dmux_pamf_context::demuxer::release_au(u32 stream_id, u32 private_stream_id, u32 au_size) const
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
inline void dmux_pamf_context::demuxer::notify_demux_done()
{
	if (!demux_done_notified && !ctx.send_event(DmuxPamfEventType::demux_done))
	{
		ctx.event_queue_too_full = true;
		return;
	}

	demux_done_notified = true;
}

void dmux_pamf_context::demuxer::demux_next_pack()
{
	if (demux_done)
	{
		return notify_demux_done();
	}
	...
	case state::initial:
	{
		...
		std::span<const std::byte, PACK_SIZE> pack = input_stream.get_next_pack();
		for (;; pack = input_stream.get_next_pack())
		{
			if (input_stream.eof())
			{
				demux_done = true;
			...
			if (raw_es)
			{
				...
				{
					elementary_streams[pack_es_type_idx][0]->set_stream(pack);
				}
		...
		std::span<const std::byte> current_pes_packet = pack.subspan(PACK_STUFFING_LENGTH_OFFSET + sizeof(u8) + pack_stuffing_length);
		...
		const std::span<const std::byte> elementary_stream = current_pes_packet.subspan(pes_header_data_length, pes_packet_length - pes_header_data_length);
	...
	{
		if (!elementary_stream::is_enabled(elementary_streams[pack_es_type_idx][pack_es_channel])
			|| elementary_streams[pack_es_type_idx][pack_es_channel]->process_stream_chunk())
		{
			state = state::initial;
		}
	...
	while (thread_ctrl::state() != thread_state::aborting)
	{
		if (get_next_cmd(cmd, new_stream))
		{
			...
			case DmuxPamfCommandType::enable_es:
				...
				demuxer.enable_es(cmd.enable_es.stream_id, cmd.enable_es.private_stream_id, cmd.enable_es.is_avc, { cmd.enable_es.au_queue_buffer.get_ptr(), cmd.enable_es.au_queue_buffer_size },
					cmd.enable_es.au_max_size, cmd.enable_es.is_raw_es, cmd.enable_es.es_id);
				break;
			...
			case DmuxPamfCommandType::release_au:
				demuxer.release_au(cmd.release_au.stream_id, cmd.release_au.private_stream_id, cmd.release_au.au_size);
				break;
			...
			case DmuxPamfCommandType::reset_es:
				demuxer.reset_es(cmd.reset_es.stream_id, cmd.reset_es.private_stream_id, cmd.reset_es.au_addr ? cmd.reset_es.au_addr.get_ptr() : nullptr);
				break;
		...
		demuxer.demux_next_pack();
	}
}

void dmux_pamf_context::elementary_stream::save(utils::serial& ar)
{
	ar(state, es_id, vm::get_addr(stream.data()), static_cast<u32>(stream.size()), pts, dts, rap);
	au_queue.save(ar);
...
{
	ar(state, demux_done, demux_done_notified, pack_es_type_idx, raw_es);
...
void DmuxPamfContext::exec(ppu_thread& ppu)
{
	...
	for (;;)
	{
		...
		// TODO Sometimes lv2 mutex/cond syscalls throw lv2 sleep failed exceptions here. Calling cpu_thread::check_state() here seems to fix that mostly? However, this appears to cause ppu_thread::loaded_from_savestate not being set correctly when loading from a savestate
		if (ppu.check_state())
		{
			ppu.state += cpu_flag::again;
			return;
...
error_code dmuxPamfEsNotifyAuFound(ppu_thread& ppu, [[maybe_unused]] vm::ptr<void> core_handle, vm::cptr<DmuxPamfAuInfo> au_info, vm::ptr<CellDmuxPamfEsHandle> handle)
{
	...
	// DmuxAuInfo::info.auMaxSize is left uninitialized
