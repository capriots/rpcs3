...
void dmux_pamf_context::elementary_stream::access_unit_queue::append_au_fragment(const access_unit_fragment& au_fragment)
{
	std::ranges::copy(au_fragment.cached_data, pos);
	std::ranges::copy(au_fragment.data, pos + au_fragment.cached_data.size());
	pos += au_fragment.data.size() + au_fragment.cached_data.size();
}

void dmux_pamf_context::elementary_stream::access_unit_queue::release_au(u32 au_size)
{
	released_pos += au_size;

	if (end != buffer.begin() && end <= released_pos)
	{
		ensure(end == released_pos);
		released_pos =
		end = buffer.begin();
	}
}
...
void dmux_pamf_context::elementary_stream::reset_es(vm::ptr<std::byte> au_addr)
{
	...
	{
		au_queue.discard_au(std::span<std::byte>::iterator(au_addr.get_ptr()));
	}
}

void dmux_pamf_context::elementary_stream::discard_access_unit()
{
	au_queue.discard_au(au.size - (au_fragment.data.size() + au_fragment.cached_data.size()));
...
bool dmux_pamf_context::elementary_stream::parse_stream()
{
	for (;;)
	{
		...
		case state::initial:
			...
			ensure(au.state != access_unit::state::size_mismatch, "Size mismatch between distance of sync words and au-size specified in stream");

			au.size += au_fragment.data.size() + au_fragment.cached_data.size();

			if (!au.timestamps_rap_set && (au.state == access_unit::state::commenced || au.state == access_unit::state::m2v_sequence || (au.state == access_unit::state::complete && au_fragment.cached_data.empty())))
			{
		...
		case state::notifying_au_found:
			if (au.state == access_unit::state::complete && !ctx.send_event(DmuxPamfEventType::au_found, stream_id, private_stream_id, vm::get_addr(au_queue.get_au_addr(au.size)),
				std::bit_cast<CellCodecTimeStamp>(static_cast<be_t<u64>>(au.pts)), std::bit_cast<CellCodecTimeStamp>(static_cast<be_t<u64>>(au.dts)), 0, au.size, au_specific_info_size, au.au_specific_info_buf, es_id, au.rap))
			{
		...
		case state::waiting_for_au_released:
			if (au.state == access_unit::state::complete && !au_queue.check_size(au_max_size))
			{
				if (au_queue.is_wrapped())
				{
					ctx.au_queue_full = true;
			...
			if (au.state == access_unit::state::complete)
			{
				au.reset();
			}
		}
...
usz dmux_pamf_context::video_stream<is_avc>::get_next_au_fragment(const std::span<const std::byte>& stream)
{
	if (au.state != access_unit::state::none && (is_avc || au.state != access_unit::state::m2v_sequence))
	{
		au.state = access_unit::state::active;
	}
	...
	[&]
	{
		for (; cache_idx <= static_cast<s64>(au.cache.size() + std::min<usz>(stream.size(), sizeof(u32) - 1) - sizeof(u32)); cache_idx++)
		{
			if (const be_t<u32> code = read_from_ptr<be_t<u32>>(buf.data() + cache_idx); (is_avc && code == AVC_AU_DELIMITER) || (!is_avc && (code == M2V_PIC_START || code == M2V_SEQUENCE_HEADER || code == M2V_SEQUENCE_END)))
			{
				if (au.state != access_unit::state::none && (is_avc || au.state != access_unit::state::m2v_sequence))
				{
					...
					au.state = access_unit::state::complete;
					return;
				}

				// TODO fist code in cache ????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????

				au.state = is_avc || code == M2V_PIC_START ? access_unit::state::commenced : access_unit::state::m2v_sequence;
			}
		}
		...
		for (; stream_it <= stream.end() - sizeof(u32); stream_it++)
		{
			if (const be_t<u32> code = read_from_ptr<be_t<u32>>(std::to_address(stream_it)); (is_avc && code == AVC_AU_DELIMITER) || (!is_avc && (code == M2V_PIC_START || code == M2V_SEQUENCE_HEADER || code == M2V_SEQUENCE_END)))
			{
				...
				au_fragment_begin = is_avc || au.state == access_unit::state::none ? stream_it : au_fragment_begin;
				au.state = is_avc || code == M2V_PIC_START ? access_unit::state::commenced : access_unit::state::m2v_sequence;
			}
		}
	}();
	...
	if (au.state != access_unit::state::none)
	{
	...
	return (au.state != access_unit::state::complete ? stream.end() : stream_it) - stream.begin();
}

usz dmux_pamf_context::lpcm_stream::get_next_au_fragment(const std::span<const std::byte>& stream)
{
	if (au.state == access_unit::state::none)
	{
		au.au_specific_info_buf = au_specific_info_buf;
	}

	if (au_max_size - au.size > stream.size())
	{
		au_fragment.data = stream;
		au.state = au.state == access_unit::state::none ? access_unit::state::commenced : access_unit::state::active;
	}
	else
	{
		au_fragment.data = stream.first(au_max_size - au.size);
		au.state = access_unit::state::complete;
	}

	return au_fragment.data.size();
}

template <bool is_ac3>
usz dmux_pamf_context::audio_stream<is_ac3>::get_next_au_fragment(const std::span<const std::byte>& stream)
{
	const auto parse_au_size = [](be_t<u16> data) -> u16
	{
		if constexpr (is_ac3)
		{
			if (u8 fscod = data >> 14, frmsizecod = (data >> 8) & 0x3f; fscod < 3 && frmsizecod < 38)
			{
		...
		for (; cache_idx <= static_cast<s64>(au.cache.size() + std::min<usz>(stream.size(), sizeof(u16) - 1) - sizeof(u16)); cache_idx++)
		{
			...
			else if ((is_ac3 && tmp == AC3_SYNCWORD) || (!is_ac3 && tmp == ATRACX_SYNCWORD))
			{
				if (au.state == access_unit::state::none)
				{
					...
					au.state = access_unit::state::commenced;
				}
				else if (u32 au_size = au.size + cache_idx; au_size >= au.parsed_size)
				{
					au.state = au_size == au.parsed_size ? access_unit::state::complete : access_unit::state::size_mismatch;
					return;
				}
			}
		}
...
usz dmux_pamf_context::user_data_stream::get_next_au_fragment(const std::span<const std::byte>& stream)
{
	if (au_size > stream.size())
	{
		au_fragment.data = stream;
		au_size -= stream.size();
		au.state = access_unit::state::commenced; // User data always uses commenced
	}
	else
	{
...
void dmux_pamf_context::enable_es(u32 stream_id, u32 private_stream_id, bool is_avc, u32 au_queue_buffer_size, vm::ptr<std::byte> au_queue_buffer, u32 au_max_size, bool is_raw_es, u32 es_id)
{
	...
	case DMUX_PAMF_STREAM_TYPE_INDEX_VIDEO:
		elementary_streams[0][channel] = is_avc ? static_cast<std::unique_ptr<elementary_stream>>(std::make_unique<video_stream<true>>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_queue_buffer_size, au_max_size))
			: std::make_unique<video_stream<false>>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_queue_buffer_size, au_max_size);
		return;

	case DMUX_PAMF_STREAM_TYPE_INDEX_LPCM:      elementary_streams[1][channel] = std::make_unique<lpcm_stream>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_queue_buffer_size, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_AC3:       elementary_streams[2][channel] = std::make_unique<audio_stream<true>>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_queue_buffer_size, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_ATRACX:    elementary_streams[3][channel] = std::make_unique<audio_stream<false>>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_queue_buffer_size, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_USER_DATA: elementary_streams[4][channel] = std::make_unique<user_data_stream>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_queue_buffer_size, au_max_size); return;
	default: fmt::throw_exception("Unreachable");
	}
}

void dmux_pamf_context::disable_es(u32 stream_id, u32 private_stream_id)
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
void dmux_pamf_context::release_au(u32 au_size, u32 stream_id, u32 private_stream_id) const
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
void dmux_pamf_context::flush_es(u32 stream_id, u32 private_stream_id)
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
{
	for (const std::unique_ptr<elementary_stream> (&types)[0x10] : elementary_streams)
	{
		std::ranges::for_each(types, [](const std::unique_ptr<elementary_stream>& es){ if (elementary_stream::is_enabled(es)) es->discard_access_unit(); });
	}
...
void dmux_pamf_context::reset_es(u32 stream_id, u32 private_stream_id, vm::ptr<std::byte> au_addr)
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
inline bool dmux_pamf_context::reset_state_and_check_demux_done()
{
	state = state::initial;
	au_queue_full = false;
...
bool dmux_pamf_context::demux(const DmuxPamfStreamInfo* stream_info)
{
	if (demux_done)
	{
		if (stream_info)
		{
			...
			state = state::initial;
			input_stream.emplace(stream_info->stream_addr, stream_info->stream_size);
			demux_done = false;
	...
	case state::initial:
	{
		for (;;)
		{
			...
			pack.stream = input_stream->get_next_pack();

			if (raw_es)
			{
				pack.type_idx = es_type_idx;
				pack.channel = 0;

				if (elementary_stream::is_enabled(elementary_streams[es_type_idx][0]))
				{
					elementary_streams[es_type_idx][0]->set_stream(pack.stream);
				}
			...
			if (const be_t<u32> code = read_from_ptr<be_t<u32>>(pack.stream); code == PACK_START)
			{
				break;
			}
...
// case demuxer::state::parsing_pack_header:
		...
		const u32 pack_stuffing_length = read_from_ptr<u8>(pack.stream.begin() + PACK_STUFFING_LENGTH_OFFSET) & 0x7;

		auto pes_packet = pack.stream.subspan(pack_stuffing_length + PACK_STUFFING_LENGTH_OFFSET + sizeof(u8));

// case demuxer::state::verifying_start_of_pes_packet: TODO
...
// case demuxer::state::parsing_system_header: TODO

		if (read_from_ptr<be_t<u32>>(pes_packet) == SYSTEM_HEADER)
		{
			const u16 system_header_length = read_from_ptr<be_t<u16>>(pes_packet.begin() + PES_PACKET_LENGTH_OFFSET) + PES_PACKET_LENGTH_OFFSET + sizeof(u16);

			ensure(pes_packet.size() >= system_header_length); // Not checked on LLE
			pes_packet = pes_packet.subspan(system_header_length); // u32 start code + u16 header length fields

			// The SPU thread isn't doing load + rotate here for 4-byte loading (for valid PAMF streams, the next start code after a system header is always 0x10 aligned)
			const auto addr_low = reinterpret_cast<std::intptr_t>(pes_packet.data()) & 0xf;
			pes_packet = { pes_packet.begin() - addr_low, pes_packet.size() + addr_low };

			ensure(pes_packet.size() >= sizeof(u32) + sizeof(u16)); // Not checked on LLE
			...
			{
				const u16 pes_packet_length = read_from_ptr<be_t<u16>>(pes_packet.begin() + PES_PACKET_LENGTH_OFFSET) + PES_PACKET_LENGTH_OFFSET + sizeof(u16);

				if (const u8 channel = read_from_ptr<u8>(pes_packet.begin() + 7) & 0xf; elementary_stream::is_enabled(elementary_streams[0][channel]))
				{
					elementary_streams[0][channel]->set_rap();
				}

				ensure(pes_packet.size() >= pes_packet_length); // Not checked on LLE
				pes_packet = pes_packet.subspan(pes_packet_length);
			}
		}

		ensure(pes_packet.size() >= sizeof(u32)); // Not checked on LLE
		const u32 pes_packet_start_code = read_from_ptr<be_t<u32>>(pes_packet);
...
// case demuxer::state::parsing_pes_packet_header: TODO

		ensure(pes_packet.size() >= 8 + sizeof(u8)); // Not checked on LLE
		const u16 pes_packet_length = read_from_ptr<be_t<u16>>(pes_packet.begin() + PES_PACKET_LENGTH_OFFSET) + PES_PACKET_LENGTH_OFFSET + sizeof(u16);
		const u8 pes_header_data_length = read_from_ptr<u8>(pes_packet.begin() + 8) + 8 + sizeof(u8);
		ensure(pes_packet.size() >= pes_packet_length); // Not checked on LLE

		const auto elementary_stream = pes_packet.subspan(pes_header_data_length, pes_packet_length - pes_header_data_length);

// case demuxer::state::parsing_elementary_stream_header: TODO

		ensure(elementary_stream.size() >= sizeof(u8)); // Not checked on LLE
		...
		{
			const s8 pts_dts_flag = read_from_ptr<s8>(pes_packet.begin() + 7);

			if (pts_dts_flag < 0)
			{
				const s32 PTS_32_30 = read_from_ptr<u8>(pes_packet.begin() + 9) >> 1;
				const s32 PTS_29_15 = read_from_ptr<be_t<u16>>(pes_packet.begin() + 10) >> 1;
				const s32 PTS_14_0 = read_from_ptr<be_t<u16>>(pes_packet.begin() + 12) >> 1;
			...
			{
				const s32 DTS_32_30 = read_from_ptr<u8>(pes_packet.begin() + 14) >> 1;
				const s32 DTS_29_15 = read_from_ptr<u16>(pes_packet.begin() + 15) >> 1;
				const s32 DTS_14_0 = read_from_ptr<u16>(pes_packet.begin() + 17) >> 1;
			...
			ensure(elementary_stream.size() >= stream_header_size); // Not checked on LLE
	...
	while (thread_ctrl::state() != thread_state::aborting)
	{
		if (get_next_cmd(cmd, new_stream))
		{
			...
			case DmuxPamfCommandType::enable_es:
				...
				enable_es(cmd.enable_es.stream_id, cmd.enable_es.private_stream_id, cmd.enable_es.is_avc, cmd.enable_es.au_queue_buffer_size, cmd.enable_es.au_queue_buffer, cmd.enable_es.au_max_size, cmd.enable_es.is_raw_es, cmd.enable_es.es_id);
				break;
			...
			case DmuxPamfCommandType::release_au:
				...
				release_au(cmd.release_au.au_size, cmd.release_au.stream_id, cmd.release_au.private_stream_id);
				break;
			...
			case DmuxPamfCommandType::reset_es:
				reset_es(cmd.reset_es.stream_id, cmd.reset_es.private_stream_id, vm::ptr<std::byte, u64>::make(cmd.reset_es.au_addr));
...
error_code DmuxPamfElementaryStream::release_au(ppu_thread& ppu, vm::ptr<void> au_addr, u32 au_size) const
{
	auto& ar = *ppu.optional_savestate_state;
	const u8 savestate = ar.try_read<u8>().second;
	ar.clear();
	...
	label1_wait_for_spu_state:
	demuxer->send_spu_command_and_wait<DmuxPamfCommandType::release_au>(ppu, savestate, au_addr.addr(), au_size, static_cast<be_t<u32>>(stream_id), static_cast<be_t<u32>>(private_stream_id));
...
error_code _CellDmuxCoreOpReleaseAu(ppu_thread& ppu, vm::ptr<CellDmuxPamfEsHandle> esHandle, vm::ptr<void> auAddr, u32 auSize)
{
