...
void dmux_pamf_base::output_queue::pop_back(std::byte* addr)
{
...
void dmux_pamf_base::elementary_stream::flush_es()
{
	if (current_au.accumulated_size != 0)
	{
		...
		interface.au_found(stream_id, private_stream_id, es_id, {au_queue.peek_back(current_au.accumulated_size), current_au.accumulated_size }, current_au.pts, current_au.dts, current_au.rap, au_specific_info_size, current_au.au_specific_info_buf);
	}

	reset();

	while (!interface.flush_done(stream_id, private_stream_id, es_id)) {}
}

void dmux_pamf_base::elementary_stream::reset_es(std::byte* au_addr)
{
	if (!au_addr)
	{
		reset();
...
u32 dmux_pamf_base::elementary_stream::parse_audio_stream_header(std::span<const std::byte> elementary_stream)
{
...
bool dmux_pamf_base::elementary_stream::process_partial_stream()
{
	for (;;)
	{
		switch (state)
		...
		case state::notifying_au_found:
			if (current_au.state == access_unit::state::complete && !interface.au_found(stream_id, private_stream_id, es_id, {au_queue.peek_back(current_au.accumulated_size), current_au.accumulated_size},
...
template <bool avc>
u32 dmux_pamf_base::video_stream<avc>::parse_stream(std::span<const std::byte> stream)
{
...
u32 dmux_pamf_base::lpcm_stream::parse_stream_header(std::span<const std::byte> elementary_stream, [[maybe_unused]] s8 pts_dts_flag)
{
	// Not checked on LLE
...
u32 dmux_pamf_base::lpcm_stream::parse_stream(std::span<const std::byte> stream)
{
	if (current_au.state == access_unit::state::none)
	{
...
template <bool ac3>
u32 dmux_pamf_base::audio_stream<ac3>::parse_stream(std::span<const std::byte> stream)
{
...
u32 dmux_pamf_base::user_data_stream::parse_stream_header(std::span<const std::byte> elementary_stream, s8 pts_dts_flag)
{
...
u32 dmux_pamf_base::user_data_stream::parse_stream(std::span<const std::byte> stream)
{
...
void dmux_pamf_base::enable_es(u32 stream_id, u32 private_stream_id, bool is_avc, std::span<std::byte> au_queue_buffer, u32 au_max_size, bool raw_es, u32 es_id)
{
	...
	case DMUX_PAMF_STREAM_TYPE_INDEX_VIDEO:
		elementary_streams[0][channel] = is_avc ? static_cast<std::unique_ptr<elementary_stream>>(std::make_unique<video_stream<true>>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size))
			: std::make_unique<video_stream<false>>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size);
		return;

	case DMUX_PAMF_STREAM_TYPE_INDEX_LPCM:      elementary_streams[1][channel] = std::make_unique<lpcm_stream>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_AC3:       elementary_streams[2][channel] = std::make_unique<audio_stream<true>>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_ATRACX:    elementary_streams[3][channel] = std::make_unique<audio_stream<false>>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size); return;
	case DMUX_PAMF_STREAM_TYPE_INDEX_USER_DATA: elementary_streams[4][channel] = std::make_unique<user_data_stream>(*this, stream_id, private_stream_id, es_id, au_queue_buffer, au_max_size); return;
	default: fmt::throw_exception("Unreachable");
	}
}

void dmux_pamf_base::disable_es(u32 stream_id, u32 private_stream_id)
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
void dmux_pamf_base::release_au(u32 stream_id, u32 private_stream_id, u32 au_size) const
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
void dmux_pamf_base::flush_es(u32 stream_id, u32 private_stream_id)
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
void dmux_pamf_base::set_stream(std::span<const std::byte> stream, bool continuity)
{
	if (!continuity)
	{
...
void dmux_pamf_base::reset_es(u32 stream_id, u32 private_stream_id, std::byte* au_addr)
{
	const auto [type_idx, channel] = dmuxPamfStreamIdToTypeChannel(stream_id, private_stream_id);
...
void dmux_pamf_base::demux_next_pack()
{
	if (!stream)
	{
		demux_done_notified = demux_done_notified || demux_done();
		return;
	}
	...
	case state::initial:
	{
		...
		std::span<const std::byte, PACK_SIZE> pack{ static_cast<const std::byte*>(nullptr), PACK_SIZE }; // This initial value is not used, can't be default constructed
		for (;; pack = stream->get_next_pack())
		{
			if (stream->eof())
			{
				stream.reset();
				demux_done_notified = demux_done();
				return;
			}
	...
	{
		if (!elementary_stream::is_enabled(elementary_streams[pack_es_type_idx][pack_es_channel]) || elementary_streams[pack_es_type_idx][pack_es_channel]->process_partial_stream())
		{
			state = state::initial;
		}
...
void dmux_pamf_base::elementary_stream::save(utils::serial& ar) const
{
...
void dmux_pamf_base::save_internal(utils::serial& ar)
{
