	...
	for (;; skip_waiting_for_au_released_or_stream_reset = false /* Savestates */)
	{
		...
		case DmuxPamfEventType::au_found:
		{
			...
			if (!stream_reset_in_progress)
			{
				...
				if (es->notify_au_found(ppu, es->_this, au_info) != CELL_OK)
				{
			...
			else if (es->skip_next_au)
			{
					...
					|| !cmd_result_queue.pop(result) || result != static_cast<u32>(DmuxPamfCmdType::reset_es) + 1)
				{
					lock.unlock();
					event_queue.pop();
					notify_fatal_err(ppu, _this, CELL_OK);
					continue;
				}
			}
		...
		case DmuxPamfEventType::demux_done:
		{
			...
			notify_demux_done(ppu, _this, CELL_OK);
		...
		case DmuxPamfEventType::flush_done:
		{
			...
			{
				lock.unlock();
				es->notify_flush_done(ppu, es->_this);
			}
		...
		{
			notify_prog_end_code(ppu, _this);
			break;
		}
		case DmuxPamfEventType::fatal_error:
		{
			event_queue.pop();
			notify_fatal_err(ppu, _this, CELL_OK); // LLE uses CELL_OK
			continue;
		}
		default:
	...
	const be_t<u32> au_max_size = [&]() -> be_t<u32>
	{
		...
		case DMUX_PAMF_STREAM_TYPE_INDEX_VIDEO:
			if (is_avc)
			{
				...
				default: utils::unreachable(); // es_specific_info was already checked for invalid values in dmuxPamfVerifyEsSpecificInfo()
				}
			}
		...
		default: utils::unreachable(); // stream_type was already checked
		}
	}();
...
{
	handle->notify_demux_done(ppu, handle, dmuxPamfGetError(error));
	return CELL_OK;
}
...
error_code dmuxPamfNotifyProgEndCode(ppu_thread& ppu, vm::ptr<void> /*core_handle*/, vm::ptr<CellDmuxPamfHandle> handle)
{
	...
	{
		handle->notify_prog_end_code(ppu, handle);
	}
...
{
	handle->notify_fatal_err(ppu, handle, dmuxPamfGetError(error));
	return CELL_OK;
}
...
error_code _CellDmuxCoreOpQueryAttr(vm::cptr<CellDmuxPamfSpecificInfo> pamfSpecificInfo, vm::ptr<CellDmuxPamfAttr> pamfAttr)
{
	...
	pamfAttr->max_enabled_es_num = 0x40;
	pamfAttr->version = DMUX_PAMF_VERSION;
	pamfAttr->mem_size = sizeof(CellDmuxPamfHandle) + sizeof(DmuxPamfContext) + 0xe7b;
...
error_code _CellDmuxCoreOpSetStream(ppu_thread& ppu, vm::ptr<CellDmuxPamfHandle> handle, vm::cptr<void> streamAddress, u32 streamSize, b8 discontinuity, u64 userData)
{
	...
	cellDmuxPamf.notice("_CellDmuxCoreOpSetStream<rawEs=%d>(handle=*0x%x, streamAddress=*0x%x, streamSize=0x%x, discontinuity=%d, userData=0x%llx)", rawEs, handle, streamAddress, streamSize, +discontinuity, userData);
	...
	return dmuxPamfGetError(handle->demuxer->set_stream(streamAddress, streamSize, discontinuity, static_cast<u32>(userData), rawEs));
}
...
u32 dmuxPamfGetEsAttr(u16 stream_id, u16 private_stream_id, bool is_avc, vm::cptr<void> es_specific_info, bool is_raw_es, CellDmuxPamfEsAttr& attr)
{
	...
	attr.au_queue_max_size = dmuxPamfGetAuQueueMaxSize(stream_id, private_stream_id);
	attr.mem_size = dmuxPamfGetEsMemSize(stream_id, private_stream_id, is_avc, es_specific_info, is_raw_es);
	attr.specific_info_size = dmuxPamfGetAuSpecificInfoSize(stream_id, private_stream_id, is_avc, is_raw_es);
...
{
	cellDmuxPamf.notice("_CellDmuxCoreOpQueryEsAttr<rawEs=%d>(esFilterId=*0x%x, esSpecificInfo=*0x%x, attr=*0x%x)", rawEs, esFilterId, esSpecificInfo, attr);
	...
	const error_code ret = dmuxPamfGetError(dmuxPamfGetEsAttr(stream_id, private_stream_id, is_avc, esSpecificInfo, rawEs, es_attr));

	*attr = es_attr;
	attr->mem_size += static_cast<u32>(sizeof(CellDmuxPamfEsHandle));
	...
	vm::cptr<DmuxCb<DmuxEsNotifyFlushDone>> notifyFlushDone, vm::cptr<void> esSpecificInfo, vm::pptr<CellDmuxPamfEsHandle> esHandle)
{
	...
	cellDmuxPamf.notice("_CellDmuxCoreOpEnableEs<rawEs=%d>(handle=*0x%x, esFilterId=*0x%x, esResource=*0x%x, notifyAuFound=*0x%x, notifyFlushDone=*0x%x, esSpecificInfo=*0x%x, esHandle)",
		rawEs, handle, esFilterId, esResource, notifyAuFound, notifyFlushDone, esSpecificInfo, esHandle);
	...
	const u32 error = handle->demuxer->enable_es(stream_id, private_stream_id, is_avc, esSpecificInfo, vm::ptr<void>::make(esResource->memAddr.addr() + sizeof(CellDmuxPamfEsHandle)),
		esResource->memSize - sizeof(CellDmuxPamfEsHandle), au_found_func, es_handle, flush_done_func, es_handle, rawEs, es_handle->es);
...
static void init_gvar(vm::gvar<CellDmuxCoreOps>& var)
{
