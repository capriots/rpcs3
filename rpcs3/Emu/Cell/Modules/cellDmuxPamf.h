...
{
	// Interface for the demuxer. These functions correspond to the events that the SPU thread sends to the PPU thread on LLE (except au_queue_full())
	virtual bool au_found(u8 stream_id, u8 private_stream_id, u32 es_id, std::span<const std::byte> au, u64 pts, u64 dts, bool rap, u8 au_specific_info_size, std::array<std::byte, 0x10> au_specific_info_buf) = 0;
	virtual bool demux_done() = 0;
	virtual void fatal_error() = 0;
	virtual bool flush_done(u8 stream_id, u8 private_stream_id, u32 es_id) = 0;
	virtual bool prog_end_code() = 0;
	virtual void au_queue_full() = 0;
	...
	public:
		explicit output_queue(std::span<std::byte> buffer) : buffer(buffer) {}
	...
	class elementary_stream
	{
		...
		std::span<const std::byte> stream; // Partial elementary stream to be processed
	...
	private:
		...
		virtual u32 parse_stream(std::span<const std::byte> stream) = 0;
	...
	protected:
		...
		u32 parse_audio_stream_header(std::span<const std::byte> elementary_stream);

	public:
		elementary_stream(dmux_pamf_base& interface, u8 stream_id, u8 private_stream_id, u32 es_id, u8 au_specific_info_size, std::span<std::byte> au_queue_buffer, u32 au_max_size)
			: interface(interface)
			, stream_id(stream_id)
		...
		void save(utils::serial& ar) const;
		...
		virtual u32 parse_stream_header(std::span<const std::byte> elementary_stream, s8 pts_dts_flag) = 0;

		// Processes the current stream section. Returns true if the stream section has been entirely consumed
		bool process_partial_stream();
	...
	{
		u32 parse_stream(std::span<const std::byte> stream) override;
		u32 parse_stream_header([[maybe_unused]] std::span<const std::byte> elementary_stream, [[maybe_unused]] s8 pts_dts_flag) override { return 0; }

	public:
		video_stream(dmux_pamf_base& interface, u32 stream_id, u32 private_stream_id, u32 es_id, std::span<std::byte> au_queue_buffer, u32 au_max_size)
			: elementary_stream(interface, stream_id, private_stream_id, es_id, 0, au_queue_buffer, au_max_size) {}
	...
	{
		u32 parse_stream(std::span<const std::byte> stream) override;
		u32 parse_stream_header(std::span<const std::byte> elementary_stream, [[maybe_unused]] s8 pts_dts_flag) override;

	public:
		lpcm_stream(dmux_pamf_base& interface, u32 stream_id, u32 private_stream_id, u32 es_id, std::span<std::byte> au_queue_buffer, u32 au_max_size)
			: elementary_stream(interface, stream_id, private_stream_id, es_id, 3, au_queue_buffer, au_max_size) {}
	...
	class audio_stream final : public elementary_stream
	{
		...
		u32 parse_stream(std::span<const std::byte> stream) override;
		u32 parse_stream_header(std::span<const std::byte> elementary_stream, [[maybe_unused]] s8 pts_dts_flag) override { return parse_audio_stream_header<0xffff>(elementary_stream); }

	public:
		audio_stream(dmux_pamf_base& interface, u32 stream_id, u32 private_stream_id, u32 es_id, std::span<std::byte> au_queue_buffer, u32 au_max_size)
			: elementary_stream(interface, stream_id, private_stream_id, es_id, 0, au_queue_buffer, au_max_size) {}
	...
	{
		u32 parse_stream(std::span<const std::byte> stream) override;
		u32 parse_stream_header(std::span<const std::byte> elementary_stream, s8 pts_dts_flag) override;

	public:
		user_data_stream(dmux_pamf_base& interface, u32 stream_id, u32 private_stream_id, u32 es_id, std::span<std::byte> au_queue_buffer, u32 au_max_size)
			: elementary_stream(interface, stream_id, private_stream_id, es_id, 0, au_queue_buffer, au_max_size) {}
	...
	class input_stream
	{
		...
		std::span<const std::byte>::iterator pos = stream.begin();

	public:
		explicit input_stream(std::span<const std::byte> stream) : stream(stream) {}
		explicit input_stream(utils::serial& ar) : stream{ vm::_ptr<const std::byte>(ar.pop<vm::addr_t>()), ar.pop<u32>() }, pos(vm::_ptr<const std::byte>(ar.pop<vm::addr_t>())) {}
...
protected:
	...
	void save_internal(utils::serial& ar);
...
public:
	...
	void enable_es(u32 stream_id, u32 private_stream_id, bool is_avc, std::span<std::byte> au_queue_buffer, u32 au_max_size, bool raw_es, u32 es_id);
	...
	void set_stream(std::span<const std::byte> stream, bool continuity);
...
class dmux_pamf_spu_context : dmux_pamf_base
{
	...
	bool au_found(u8 stream_id, u8 private_stream_id, u32 es_id, std::span<const std::byte> au, u64 pts, u64 dts, bool rap, u8 au_specific_info_size, std::array<std::byte, 0x10> au_specific_info_buf) override
	{
		return !((wait_for_event_queue = !send_event(DmuxPamfEventType::au_found, stream_id, private_stream_id, vm::get_addr(au.data()), std::bit_cast<CellCodecTimeStamp>(static_cast<be_t<u64>>(pts)),
			std::bit_cast<CellCodecTimeStamp>(static_cast<be_t<u64>>(dts)), 0, static_cast<u32>(au.size()), au_specific_info_size, au_specific_info_buf, es_id, rap)));
	}

	bool demux_done() override { return !((wait_for_event_queue = !send_event(DmuxPamfEventType::demux_done))); }
	void fatal_error() override { send_event(DmuxPamfEventType::fatal_error); }
	bool flush_done(u8 stream_id, u8 private_stream_id, u32 es_id) override { return send_event(DmuxPamfEventType::flush_done, stream_id, private_stream_id, es_id); } // The "flush done" event does not set wait_for_event_queue if the queue is full
	bool prog_end_code() override { return !((wait_for_event_queue = !send_event(DmuxPamfEventType::prog_end_code))); }
	void au_queue_full() override { wait_for_au_queue = true; }
