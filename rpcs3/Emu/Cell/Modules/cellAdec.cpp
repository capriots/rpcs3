		...
		case LpcmDecCmdType::decode_au:
		{
			...
			if (!dvd_packing)
			{
				...
				else if (lpcm_param->sizeOfWord == CELL_ADEC_BIT_LENGTH_20 || lpcm_param->sizeOfWord == CELL_ADEC_BIT_LENGTH_24)
				{
					s64 byte_idx = 0;
					s64 sample_idx = 0;
					for (; byte_idx <= au_size_u8 - static_cast<s64>(sizeof(v128)); byte_idx += 4ll * 3, sample_idx += 4)
					{
					...
					for (; byte_idx <= au_size_u8 - 3; byte_idx += 3, sample_idx++)
					{
				...
				// The following loops can access up to 7 bytes past the output buffer if sample_num % channel_num != 0. This is intended, the buffer is larger than LPCM_DEC_OUTPUT_BUFFER_SIZE (see _CellAdecCoreOpGetMemSize_lpcm())
				switch (lpcm_param->channelNumber)
				{
				...
				case CELL_ADEC_CH_3_0:
					for (s32 i_in = 0, i_out = 0; i_in < sample_num; i_in += 4, i_out += 3)
					{
						...
						tmp = gv_shuffle32<0 << 0 | 2 << 2 | 1 << 4 | 3 << 6>(tmp); // Swap Front Right and Center
				...
				case CELL_ADEC_CH_3_4:
					for (s32 i_in = 0, i_out = 0; i_in < sample_num; i_in += 8, i_out += 7)
					{
						...
						tmp1 = gv_shuffle32<0 << 0 | 2 << 2 | 1 << 4 | 3 << 6>(tmp1); // Swap Front Right and Center
						tmp2 = gv_shuffle32<2 << 0 | 0 << 2 | 1 << 4 | 3 << 6>(tmp2); // Reorder Rear Left, Rear Right, Side Right -> Side Right, Rear Left, Rear Right
				...
				case CELL_ADEC_CH_3_4_LFE:
					for (s32 i = 0; i < sample_num; i += 8)
					{
						...
						*reinterpret_cast<v128*>(&_output[i + 4]) = gv_shuffle32<3 << 0 | 2 << 2 | 0 << 4 | 1 << 6>(tmp1); // Reorder Rear Left, Rear Right, Side Right, LFE -> LFE, Side Right, Rear Left, Rear Right
			...
			else
			{
				...
				case CELL_ADEC_BIT_LENGTH_16:
				{
					...
					for (; i_in <= au_size_s16 - channel_num - 2; i_in += channel_num * 2, i_out += 4)
					{
						...
						v128 s16be = gv_loadu32(&input_s16[i_in]);
						s16be = gv_insert32<1>(s16be, read_from_ptr<u32>(&input_s16[i_in + channel_num]));
			...
			notify_pcm_out.cbFunc(ppu, cmd.pcm_handle, output, static_cast<u32>(output_size), notify_pcm_out.cbArg, vm::make_var<vm::bcptr<void>>(bsi_info), ADEC_CORRECT_PTS_VALUE_TYPE_LPCM, error_occurred ? static_cast<s32>(CELL_ADEC_ERROR_FATAL) : CELL_OK);
			break;
		}
		default:
	...
	vm::ptr<AdecNotifyError> notifyError, vm::ptr<void> notifyErrorArg, vm::ptr<AdecNotifySeqDone> notifySeqDone, vm::ptr<void> notifySeqDoneArg, vm::cptr<CellAdecResource> res, vm::cptr<CellAdecResourceSpurs> spursRes)
{
	...
	if (error_code ret = sys_mutex_create(ppu, handle.ptr(&LpcmDecContext::spurs_queue_pop_mutex), mutex_attr); ret != CELL_OK
