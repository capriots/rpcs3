...
void LpcmDecContext::exec(ppu_thread& ppu)
{
	...
	for (; run_thread; cmd_counter++)
	{
		...
		if (!skip_getting_command)
		{
			...
			{
				lv2_obj::sleep(ppu);
				cmd_available.value.wait(0, static_cast<atomic_wait_timeout>(20'000'000));
			}
		}
		...
		case LpcmDecCmdType::decode_au:
		{
			...
			if (!using_dvd_packing)
			{
				...
				{
					cellAdec.trace("Unsupported sizeOfWord %d", lpcm_param->sizeOfWord); // Trace because it would be spammed otherwise. Output_size and sample_num stay the same
				}
				...
				default: cellAdec.trace("Unsupported channelNumber %d", lpcm_param->channelNumber); // Trace because it would be spammed otherwise. Channel_num and output_size stay the same
				}
				...
				if (lpcm_param->sizeOfWord == CELL_ADEC_BIT_LENGTH_16)
				{
					...
					for (; i <= au_size_s16 - 8; i += 8)
					{
						...
						const v128 s32_1 = std::endian::native == std::endian::little
							? gv_shuffle8<v128::normal_array_t<s8>{ -1, -1, 1, 0, -1, -1, 3, 2, -1, -1, 5, 4, -1, -1, 7, 6 }>(s16be)
							: gv_unpacklo16(s16be, gv_bcst16(0));

						const v128 s32_2 = std::endian::native == std::endian::little
							? gv_shuffle8<v128::normal_array_t<s8>{ -1, -1, 9, 8, -1, -1, 11, 10, -1, -1, 13, 12, -1, -1, 15, 14 }>(s16be)
				...
				else if (lpcm_param->sizeOfWord == CELL_ADEC_BIT_LENGTH_20 || lpcm_param->sizeOfWord == CELL_ADEC_BIT_LENGTH_24)
				{
					...
					for (; byte_idx <= au_size_u8 - static_cast<s64>(sizeof(v128)); byte_idx += 4ull * 3 /*sizeof s24*/, sample_idx += 4)
					{
						...
						const v128 _s32 = std::endian::native == std::endian::little
							? gv_shuffle8<v128::normal_array_t<s8>{ -1, 2, 1, 0, -1, 5, 4, 3, -1, 8, 7, 6, -1, 11, 10, 9 }>(v128::loadu(&input_u8[byte_idx]))
							: gv_shuffle8<v128::normal_array_t<s8>{ 0, 1, 2, -1, 3, 4, 5, -1, 6, 7, 8, -1, 9, 10, 11, -1 }>(v128::loadu(&input_u8[byte_idx]));
					...
					for (; byte_idx /*sizeof s24*/ <= au_size_u8 - 3; byte_idx += 3 /*sizeof s24*/, sample_idx++)
					{
						...
						_output[sample_idx] = static_cast<f32>(std::bit_cast<le_t<s32>>(s32le)) / static_cast<f32>(0x80000000u);
					}
				}
				...
				if (channel_num != 0)
				{
					...
					default: ; // Unreachable, channel_num would be 0
					}
				}
			}
			else
			{
				...
				default: cellAdec.trace("Unsupported sizeOfWord %d", lpcm_param->sizeOfWord); // Trace because it would be spammed otherwise. Output_size stays the same // TODO what actually happens here
				}
				...
				default: fmt::throw_exception("Invalid channelNumber %d", lpcm_param->channelNumber);
				}
				...
				case CELL_ADEC_BIT_LENGTH_20:
				{
					...
					for (s64 i_in = 0, i_out = 0; i_in <= au_size_u8 - low_bits_4_offset; i_in += next_samples_offset, i_out += 4)
					{
						...
						const v128 _s32 = std::endian::native == std::endian::little
							? gv_shuffle8<v128::normal_array_t<s8>{ -1, 8, 1, 0, -1, 9, 3, 2, -1, 10, 5, 4, -1, 11, 7, 6 }>(s20be)
							: gv_shuffle8<v128::normal_array_t<s8>{ 0, 1, 8, -1, 2, 3, 9, -1, 4, 5, 10, -1, 6, 7, 11, -1 }>(s20be);
				...
				case CELL_ADEC_BIT_LENGTH_24:
				{
					...
					for (s64 i_in = 0, i_out = 0; i_in <= au_size_u8 - low_bytes_3_4_offset - 2; i_in += next_samples_offset, i_out += 4)
					{
						...
						// Reorder bytes to form four 32-bit integer samples
						const v128 _s32 = std::endian::native == std::endian::little
							? gv_shuffle8<v128::normal_array_t<s8>{ -1, 8, 1, 0, -1, 9, 3, 2, -1, 10, 5, 4, -1, 11, 7, 6 }>(s24be)
							: gv_shuffle8<v128::normal_array_t<s8>{ 0, 1, 8, -1, 2, 3, 9, -1, 4, 5, 10, -1, 6, 7, 11, -1 }>(s24be);
				...
				default:
					fmt::throw_exception(""); // TODO see above
				}
			}
...
inline error_code LpcmDecContext::send_command(ppu_thread& ppu, auto&&... args)
{
	...
	std::lock_guard lock1{queue_size_mutex};
	...
	{
		std::lock_guard lock2{queue_mutex};
...
error_code _CellAdecCoreOpClose_lpcm(ppu_thread& ppu, vm::ptr<LpcmDecContext> handle)
{
	...
	{
		std::scoped_lock lock{ handle->queue_size_mutex, handle->queue_mutex };
	...
	if (vm::var<u64> ret; sys_ppu_thread_join(ppu, handle->thread_id, +ret) != CELL_OK)
	{
		// Another thread already closed the decoder
...
error_code adecSetLpcmBlurayParams(vm::ptr<LpcmDecContext> handle, u64 userData)
{
	...
	switch (size_of_word)
	{
	case CELL_ADEC_BIT_LENGTH_16: allocated_bits_per_sample = 16; break;
	case CELL_ADEC_BIT_LENGTH_20: // Same as below
	case CELL_ADEC_BIT_LENGTH_24: allocated_bits_per_sample = 24; break;
	default: return CELL_ADEC_ERROR_FATAL;
	}
