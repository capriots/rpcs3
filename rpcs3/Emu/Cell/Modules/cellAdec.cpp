...
void LpcmDecContext::exec(ppu_thread& ppu)
{
	...
	for (; run_thread; cmd_counter++)
	{
		...
		{
			while (cmd_available.value == 0 && !ppu.is_stopped())
			{
				lv2_obj::sleep(ppu);
				cmd_available.value.wait(0, atomic_wait_timeout(20'000'000));
			}
		}

		cellAdec.trace("Command available, waiting for output to be consumed...");

		// Wait for the ouput to be consumed
		...
		{
			lv2_obj::sleep(ppu);
			output_consumed.wait(output_mutex, 20'000);
		}
		...
		// Get command and remove it from queue
		if (!skip_getting_command)
		{
			lv2_obj::sleep(ppu);
		...
		case LpcmDecCmdType::start_seq:
			// LLE sends a command to the SPU thread. The SPU thread doesn't do anything however
			break;
		...
		case LpcmDecCmdType::decode_au:
		{
			...
			const s64 au_size_s16 = au_size_u8 / sizeof(s16);
			...
			if (!using_dvd_packing)
			{
				...
				{
					output_size = output_size * 4 / 3;
					sample_num /= 3;
				}

				// LPCM streams with an odd amount of channels contain an empty dummy channel
				...
				case CELL_ADEC_CH_3_0:     channel_num = 3; output_size = output_size * 3 / 4; break;
				case CELL_ADEC_CH_2_1:     channel_num = 3; output_size = output_size * 3 / 4; break;
				case CELL_ADEC_CH_3_1:     channel_num = 4; break;
				...
				// LLE doesn't check the output size, it would just segfault if it's too large
				...
				if (lpcm_param->sizeOfWord == CELL_ADEC_BIT_LENGTH_16)
				{
					...
					for (; i <= au_size_s16 - 8; i += 8)
					{
						...
						const v128 f32_1 = gv_mulfs(gv_cvts32_tofs(s32_1), 1.f / 0x80000000u);
						const v128 f32_2 = gv_mulfs(gv_cvts32_tofs(s32_2), 1.f / 0x80000000u);

						*reinterpret_cast<v128*>(&_output[i]) = gv_to_be32(f32_1);
						*reinterpret_cast<v128*>(&_output[i + 4]) = gv_to_be32(f32_2);
					}
					for (; i < au_size_s16; i++) // This loop is probably unnecessary, compliant frames always have a multiple of 8 samples
					{
				...
				{
					s64 byte_idx = 0, sample_idx = 0;
					for (; byte_idx <= au_size_u8 - static_cast<s64>(sizeof(v128)); byte_idx += 4ull * 3 /*sizeof s24*/, sample_idx += 4)
					{
						// Load four samples, convert endianness if necessary and shift left by 8
						const v128 _s32 = std::endian::native == std::endian::little
							? gv_shuffle8(v128::loadu(&input_u8[byte_idx]), v128::normal_array_t<s8>{ -1, 2, 1, 0, -1, 5, 4, 3, -1, 8, 7, 6, -1, 11, 10, 9 })
							: gv_shuffle8(v128::loadu(&input_u8[byte_idx]), v128::normal_array_t<s8>{ 0, 1, 2, -1, 3, 4, 5, -1, 6, 7, 8, -1, 9, 10, 11, -1 });

						// Convert to float and divide by INT32_MAX + 1
						const v128 _f32 = gv_mulfs(gv_cvts32_tofs(_s32), 1.f / 0x80000000u);

						*reinterpret_cast<v128*>(&_output[sample_idx]) = gv_to_be32(_f32);
					}
					for (; byte_idx /*sizeof s24*/ <= au_size_u8 - 3; byte_idx += 3 /*sizeof s24*/, sample_idx++)
					{
						alignas(alignof(s32)) const u8 s32le[4] = { 0, input_u8[byte_idx + 2], input_u8[byte_idx + 1], input_u8[byte_idx] };

						_output[sample_idx] = static_cast<f32>(std::bit_cast<le_t<s32>>(s32le)) / 0x80000000u;
					}
				}

				// Reorder channels and remove dummy channel
				if (channel_num != 0)
				{
					// LLE doesn't check the output size, it would just segfault if it's too large
					// The following loops can read up to 7 bytes more from the output buffer than LPCM_DEC_OUTPUT_BUFFER_SIZE. This is intended, there is more than enough space past the buffer
					ensure(sample_num * sizeof(f32) <= LPCM_DEC_OUTPUT_BUFFER_SIZE);

					// Input channel order:
					// L   R   C   LS   Rls  Rrs  RS   lfe

					// Output channel order:
					// - up to 3_4   L   C   R   LS   RS   Rls  Rrs  lfe
					// - 3_4_LFE     L   R   C   lfe  LS   RS   Rls  Rrs
					...
					case CELL_ADEC_CH_3_0:
						...
						{
							v128 tmp = *reinterpret_cast<v128*>(&_output[i_in]);
							tmp = gv_shuffle32<0 << 0 | 2 << 2 | 1 << 4 | 3 << 6>(tmp); // Swap right and center channels
					...
					case CELL_ADEC_CH_3_2_LFE:
						...
						{
							const u64 tmp = std::rotl(read_from_ptr<u64>(&_output[i + 1]), 0x20); // Swap right and center channels
							write_to_ptr<u64>(&_output[i + 1], tmp);
						}
						break;
					...
					case CELL_ADEC_CH_3_2:
						...
						{
							v128 tmp = v128::loadu(&_output[i_in]);
							tmp = gv_shuffle32<0 << 0 | 2 << 2 | 1 << 4 | 3 << 6>(tmp); // Swap right and center channels
							v128::storeu(tmp, &_output[i_out]);
					...
					case CELL_ADEC_CH_3_4:
						...
						{
							v128 tmp1 = *reinterpret_cast<v128*>(&_output[i_in]);
							v128 tmp2 = *reinterpret_cast<v128*>(&_output[i_in + 4]);
							tmp1 = gv_shuffle32<0 << 0 | 2 << 2 | 1 << 4 | 3 << 6>(tmp1); // Swap right and center channels
							tmp2 = gv_shuffle32<2 << 0 | 0 << 2 | 1 << 4 | 3 << 6>(tmp2); // Reorder Rls Rrs RS -> RS Rls Rrs
					...
					case CELL_ADEC_CH_3_4_LFE:
						...
						{
							const v128 tmp1 = *reinterpret_cast<v128*>(&_output[i + 4]);
							*reinterpret_cast<v128*>(&_output[i + 4]) = gv_shuffle32<3 << 0 | 2 << 2 | 0 << 4 | 1 << 6>(tmp1); // Reorder Rls Rrs RS lfe -> lfe RS Rls Rrs
							const u64 tmp2 = std::rotl(read_from_ptr<u64>(&_output[i + 3]), 0x20); // Swap LS and lfe channels
							write_to_ptr<u64>(&_output[i + 3], tmp2);
						}
					}
				}
			}
			else
			{
				...
				default: fmt::throw_exception("Invalid channel layout: %d", lpcm_param->channelNumber);
				}
				...
				case CELL_ADEC_BIT_LENGTH_16:
				{
					...
					{
						// Load two samples and insert two more into the vector
						const v128 s16be = gv_insert32<1>(gv_loadu32(&input_s16[i_in]), read_from_ptr<u32>(&input_s16[i_in + channel_num]));
				...
				case CELL_ADEC_BIT_LENGTH_20:
				{
					...
					const s64 low_bits_3_offset = channel_num * 4 + channel_num / 2;
					const s64 low_bits_4_offset = low_bits_3_offset + (channel_num & 1);

					alignas(alignof(v128)) static constexpr v128::normal_array_t<u8> mask_same_offset      = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf0, 0x0f, 0xf0, 0x0f, 0xff, 0xff, 0xff, 0xff };
					alignas(alignof(v128)) static constexpr v128::normal_array_t<u8> mask_different_offset = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf0, 0x0f, 0x0f, 0xf0, 0xff, 0xff, 0xff, 0xff };
					const v128 low_bits_mask = low_bits_3_offset == low_bits_4_offset ? mask_same_offset : mask_different_offset;

					const s32 next_samples_offset = channel_num * 5;

					for (s64 i_in = 0, i_out = 0; i_in <= au_size_u8 - low_bits_4_offset; i_in += next_samples_offset, i_out += 4)
					{
						// Load all the high and low bits of four samples
						v128 s20be = gv_loadu32(&input_u8[i_in]);
						s20be = gv_insert32<1>(s20be, read_from_ptr<u32>(&input_u8[i_in + high_bytes_3_4_offset]));
						s20be = gv_insert8<8>(s20be, input_u8[i_in + low_bits_1_2_offset]);
						s20be = gv_insert8<9>(s20be, input_u8[i_in + low_bits_1_2_offset]);
						s20be = gv_insert8<10>(s20be, input_u8[i_in + low_bits_3_offset]);
						s20be = gv_insert8<11>(s20be, input_u8[i_in + low_bits_4_offset]);

						// Extract the correct bits for each sample
						s20be = gv_and32(s20be, low_bits_mask);

						// LLE is missing a step: each byte that was AND with 0x0f would still need to be shifted left by 4

						// Reorder bytes to form four 32-bit integer samples
						const v128 _s32 = std::endian::native == std::endian::little
							? gv_shuffle8(s20be, v128::normal_array_t<s8>{ -1, 8, 1, 0, -1, 9, 3, 2, -1, 10, 5, 4, -1, 11, 7, 6 })
							: gv_shuffle8(s20be, v128::normal_array_t<s8>{ 0, 1, 8, -1, 2, 3, 9, -1, 4, 5, 10, -1, 6, 7, 11, -1 });
				...
				case CELL_ADEC_BIT_LENGTH_24:
				{
					...
					for (s64 i_in = 0, i_out = 0; i_in <= au_size_u8 - low_bytes_3_4_offset - 2; i_in += next_samples_offset, i_out += 4)
					{
						...
						v128 s24be = gv_loadu32(&input_u8[i_in]);
						s24be = gv_insert32<1>(s24be, read_from_ptr<u32>(&input_u8[i_in + high_bytes_3_4_offset]));
			...
			notify_pcm_out.cbFunc(ppu, cmd.pcm_handle, output, static_cast<u32>(output_size), notify_pcm_out.cbArg, vm::make_var<vm::bcptr<void>>(bsi_info), ADEC_CORRECT_PTS_VALUE_TYPE_LPCM, CELL_OK);
			break;
		}
		default:
...
inline error_code LpcmDecContext::send_command(ppu_thread& ppu, auto&&... args)
{
	ppu.state += cpu_flag::wait;

	std::lock_guard lock{queue_size_mutex};
...
inline void LpcmDecContext::release_output()
{
	...
	vm::ptr<AdecNotifyError> notifyError, vm::ptr<void> notifyErrorArg, vm::ptr<AdecNotifySeqDone> notifySeqDone, vm::ptr<void> notifySeqDoneArg, vm::cptr<CellAdecResource> res, vm::cptr<CellAdecResourceSpurs> spursRes)
{
	...
	ensure(!!notifyAuDone && !!notifyAuDoneArg && !!notifyPcmOut && !!notifyPcmOutArg && !!notifyError && !!notifyErrorArg && !!notifySeqDone && !!notifySeqDoneArg); // These are always be set by LLE cellAdec

	const u32 end_of_context_addr = handle.addr() + utils::align(static_cast<u32>(sizeof(LpcmDecContext)), 0x80);
	const auto lpcm_param = vm::ptr<CellAdecParamLpcm>::make(end_of_context_addr);
	const auto spurs_cmd_data = vm::ptr<void>::make(end_of_context_addr + utils::align(static_cast<u32>(sizeof(CellAdecParamLpcm)), 0x80));
	const auto output = vm::ptr<f32>::make(end_of_context_addr + 0x180);

	new (handle.get_ptr()) LpcmDecContext(notifyAuDone, notifyAuDoneArg, notifyPcmOut, notifyPcmOutArg, notifyError, notifyErrorArg, notifySeqDone, notifySeqDoneArg, output, lpcm_param, spurs_cmd_data);
	...
	ppu_execute<&sys_ppu_thread_create>(ppu, handle.ptr(&LpcmDecContext::thread_id), entry, handle.addr(), +res->ppuThreadPriority, +res->ppuThreadStackSize, SYS_PPU_THREAD_CREATE_JOINABLE, +_name);
...
error_code _CellAdecCoreOpClose_lpcm(ppu_thread& ppu, vm::ptr<LpcmDecContext> handle)
{
	...
	{
		std::lock_guard lock1{handle->queue_size_mutex};
		std::lock_guard lock2{handle->queue_mutex};

		if (handle->cmd_queue.empty())
		{
			handle->cmd_queue.emplace(LpcmDecCmdType::close);
			handle->cmd_available.post();
		}
		else
		{
...
error_code AdecContext::get_new_pcm_handle(vm::ptr<CellAdecAuInfo> au_info)
{
...
error_code AdecContext::verify_pcm_handle(s32 pcm_handle)
{
	ensure(pcm_handle >= 0 && pcm_handle < pcm_handle_num); // Not properly checked on LLE, see below

	if ((pcm_handle < 0 && /* LLE uses && instead of || */ pcm_handle >= pcm_handle_num) || !frames[pcm_handle].in_use)
	{
		return CELL_ADEC_ERROR_FATAL;
	}
...
vm::ptr<CellAdecAuInfo> AdecContext::get_au_info(s32 pcm_handle)
{
...
void AdecContext::set_state(s32 pcm_handle, u32 state)
{
	if (state == (1u << 9))
	{
...
error_code AdecContext::get_pcm_item(s32 pcm_handle, vm::ptr<CellAdecPcmItem>& pcm_item)
{
	if (verify_pcm_handle(pcm_handle) == static_cast<s32>(CELL_ADEC_ERROR_FATAL))
	{
		return CELL_ADEC_ERROR_FATAL;
...
error_code AdecContext::set_pcm_item(s32 pcm_handle, vm::ptr<void> pcm_addr, u32 pcm_size, vm::cpptr<void> bitstream_info)
{
	if (verify_pcm_handle(pcm_handle) == static_cast<s32>(CELL_ADEC_ERROR_FATAL))
	{
		return CELL_ADEC_ERROR_FATAL;
...
void AdecContext::reset_frame(s32 pcm_handle)
{
...
error_code AdecContext::correct_pts_value(ppu_thread& ppu, s32 pcm_handle, s8 correct_pts_type)
{
	...
	case ADEC_CORRECT_PTS_VALUE_TYPE_DTSHDCORE_UNK1: if (ppu_execute<&_SceAdecCorrectPtsValue_DtsHd_Core>(ppu, +core_handle, au_pts) != CELL_OK) return CELL_ADEC_ERROR_FATAL; else return CELL_OK;
...
error_code adecNotifyAuDone(ppu_thread& ppu, s32 pcmHandle, vm::ptr<AdecContext> handle)
{
	...
	handle->callback.cbFunc(ppu, handle, CELL_ADEC_MSG_TYPE_AUDONE, au_info.addr(), handle->callback.cbArg);
...
error_code adecOpen(ppu_thread& ppu, vm::ptr<CellAdecType> type, vm::cptr<CellAdecResource> res, vm::cptr<CellAdecCb> cb, vm::pptr<AdecContext> handle, vm::cptr<CellAdecResourceSpurs> spursRes)
{
	...
	// Set LPCM packing format if decoder type is LPCM
	if (type->audioCodecType == CELL_ADEC_TYPE_LPCM_DVD)
	{
		vm::static_ptr_cast<LpcmDecContext>(core_handle)->using_dvd_packing = true;
	}
	else if (type->audioCodecType == CELL_ADEC_TYPE_LPCM_PAMF || type->audioCodecType == CELL_ADEC_TYPE_LPCM_BLURAY)
	{
		vm::static_ptr_cast<LpcmDecContext>(core_handle)->using_dvd_packing = false;
	}
...
error_code adecSetLpcmBlurayParams(vm::ptr<LpcmDecContext> handle, u64 userData)
{
	...
	switch (sample_rate)
	{
	case CELL_ADEC_FS_48kHz: kHz = 48; break;
	case CELL_ADEC_FS_96kHz: kHz = 96; break;
	case CELL_ADEC_FS_192kHz: kHz = 192; break;
	default:
		return CELL_ADEC_ERROR_FATAL;
	}

	u32 allocated_bits_per_sample;

	switch (size_of_word)
	{
	case CELL_ADEC_BIT_LENGTH_16:
		allocated_bits_per_sample = 16;
		break;

	case CELL_ADEC_BIT_LENGTH_20:
	case CELL_ADEC_BIT_LENGTH_24:
		allocated_bits_per_sample = 24;
		break;
	...
	const u32 bytes_per_second = kHz * allocated_bits_per_sample * allocated_channels / 8;
	const u32 frame_length = bytes_per_second * 5;

	handle->lpcm_param->audioPayloadSize = frame_length * unk;
...
error_code adecSetLpcmDvdParams(vm::ptr<LpcmDecContext> handle, u64 userData)
{
	...
	const u8 sample_rate = static_cast<u8>(userData >> 40);
	const u8 size_of_word = static_cast<u8>(userData >> 48);
	const u8 unk1 = static_cast<u8>(userData >> 56);
	...
	switch (sample_rate)
	{
	case CELL_ADEC_FS_48kHz: unk2 = 48 * 5 / 3; break;
	case CELL_ADEC_FS_96kHz: unk2 = 96 * 5 / 3; break;
	default:
		return CELL_ADEC_ERROR_FATAL;
	}
	...
	handle->lpcm_param->audioPayloadSize = bits_per_sample * channel_number * unk2 / 8 * unk1;
