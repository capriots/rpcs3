	...
	for (; run_thread; cmd_counter++)
	{
		...
		case LpcmDecCmdType::decode_au:
		{
			...
			if (!dvd_packing)
			{
				...
				else if (lpcm_param->sizeOfWord == CELL_ADEC_BIT_LENGTH_20 || lpcm_param->sizeOfWord == CELL_ADEC_BIT_LENGTH_24)
				{
					...
					for (; i * 3 <= au_size_u8 - static_cast<s64>(sizeof(v128)); i += 4)
					{
						...
						const v128 _s32 = std::endian::native == std::endian::little
							? gv_shuffle8<v128::normal_array_t<s8>{ -1, 2, 1, 0, -1, 5, 4, 3, -1, 8, 7, 6, -1, 11, 10, 9 }>(v128::loadu(&input_u8[i * 3]))
							: gv_shuffle8<v128::normal_array_t<s8>{ 0, 1, 2, -1, 3, 4, 5, -1, 6, 7, 8, -1, 9, 10, 11, -1 }>(v128::loadu(&input_u8[i * 3]));
				...
				// The following loops can access up to 7 bytes past the output buffer if sample_num % channel_num != 0.
				// This is intended, LLE does something similar. The buffer is larger than LPCM_DEC_OUTPUT_BUFFER_SIZE (see _CellAdecCoreOpGetMemSize_lpcm())
				switch (lpcm_param->channelNumber)
				{
