...
struct AdecFrame
{
	...
	// Frames that are ready to be consumed form a linked list. However, this doesn't serve any purpose (AdecOutputQueue is used instead)
	be_t<s32> next = 0; // Index of next frame that can be consumed
	be_t<s32> prev = 0; // Index of previous frame that can be consumed

	AdecFrame(u32 index, u32 bitstream_info_addr)
		: this_index(index)
	{
...
struct AdecContext // CellAdecHandle = AdecContext*
{
	...
	const be_t<s32> pcm_handle_num;
	const u32 reserved1 = 0;
	be_t<s32> frames_head = -1;       // Index of oldest frame that can be consumed
	be_t<s32> frames_tail = -1;       // Index of most recent frame that can be consumed
	...
	AdecContext(vm::bptr<AdecContext> _this, u32 this_size, const CellAdecType& type, const CellAdecResource& res, const CellAdecCb& callback, vm::bptr<void> core_handle, vm::bcptr<CellAdecCoreOps> core_ops,
		s32 pcm_handle_num, vm::bptr<AdecFrame> frames, u32 bitstream_info_size, u32 bitstream_infos_addr)
		: _this(_this), this_size(this_size), type(type), res(res), callback(callback), core_handle(core_handle), core_ops(core_ops), pcm_handle_num(pcm_handle_num), frames(frames), bitstream_info_size(bitstream_info_size)
	{
	...
	error_code get_new_pcm_handle(vm::ptr<CellAdecAuInfo> au_info);
	error_code verify_pcm_handle(s32 pcm_handle);
	vm::ptr<CellAdecAuInfo> get_au_info(s32 pcm_handle);
	void set_state(s32 pcm_handle, u32 state);
	error_code get_pcm_item(s32 pcm_handle, vm::ptr<CellAdecPcmItem>& pcm_item);
	error_code set_pcm_item(s32 pcm_handle, vm::ptr<void> pcm_addr, u32 pcm_size, vm::cpptr<void> bitstream_info);
	error_code link_frame(s32 pcm_handle);
	error_code unlink_frame(s32 pcm_handle);
	void reset_frame(s32 pcm_handle);
